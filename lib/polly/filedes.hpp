...
// could be provided by user code? it would work well for lambdas or full functions
// with a void(Socket&, int) signature. But the problem is, how can we access 
// derived-specific functions if it's part of the base class? this is where the 
// CTRP (Curiously recurring template pattern) enters the scene. Essentially,
// we have a template base class, and then use the template parameter to define 
// functions that work derived classes. Then, when we inherit from base, we
// pass our new class as a template parameter. Like this:
//
// `class Derived : public Base<Derived>`
//
// However, since Base is a template class, we can't just refer to it homogenously,
// so we add an AbstractBase which contains virtual functions that we want
// Epoll to be able to call (handle, get_fd) and then inherit from that in our
// template base class. It's a way to reduce the amount of copy-pasted code that needs
// to be written for every file descriptor wrapper.
// You can read more here: https://en.wikipedia.org/wiki/Curiously_recurring_template_pattern

static int dup(int fd) {
	int res = ::dup(fd);
	if (res < 0) throw std::system_error(errno, std::generic_category(), "dup() failed");
	return res;
}
...
template<typename T>
class FileDes: public AbstractFileDes {
public:
...
protected:
	...
	handler_t handler;
public:
	void set_handler(handler_t new_handler) {
		handler = new_handler;
	}
	void handle(int events) final {
		handler(static_cast<T&>(*this), events);
	}
	// copy operator
